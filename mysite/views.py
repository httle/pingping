...
def cutdown(datalist):
    for x in datalist:
        if x.content_object.canread == 0:
            datalist.remove(x)
    return datalist
...
def get_7_days_hot_blogs():
    ...
    blogs = Blog.objects \
                .filter(read_details__date__lt=today, read_details__date__gte=date,canread=1) \
                .values('id', 'title') \
...
def home(request):
    ...
    context['today_hot_data'] = cutdown(list(get_today_hot_data(blog_content_type)))
    context['yesterday_hot_data'] = cutdown(list(get_yesterday_hot_data(blog_content_type)))
