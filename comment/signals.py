...
def send_notification(sender, instance, **kwargs):
    ...
    if instance.reply_to is None:
        ...
        if instance.content_type.model == 'blog':
            ...
            verb = '{0} 评论了你的《{1}》'.format(instance.user.get_nickname_or_username(), target.title)
        else:
    ...
    description = strip_tags(instance.text)
    url = instance.content_object.get_url() + "#root_" + str(target.pk)
    if(not instance.user == recipient):
        notify.send(instance.user, recipient=recipient, verb=verb, action_object=instance, url=url,description=description)
